...
def extract_languages(section_text: Optional[str], text: str) -> List[str]:
    if section_text:
        ...
        source = "\n".join(matched)
    values = normalize_languages([source])
    if values:
        return values[:8]

    # Fallback scan from full text when no explicit language section is detected.
    candidates = []
    for ln in text.splitlines():
        lower_ln = ln.lower()
        if "language" in lower_ln or "languages" in lower_ln:
            candidates.append(ln)
    return normalize_languages(candidates)[:8]


def extract_projects(section_text: Optional[str], text: str) -> List[str]:
    candidates: List[str] = []
    if section_text:
        candidates.extend(section_text.splitlines())
    else:
        for ln in text.splitlines():
            if re.search(r"\bprojects?\b", ln, flags=re.IGNORECASE):
                candidates.append(ln)
    ...
    for keys in SECTION_HEADERS.values():
        for key in keys:
            blocked_prefixes.add(key.lower())
    for raw in candidates:
        line = re.sub(r"^[\s\u2022\-\*\d\.\)\(]+", "", str(raw or "")).strip()
        if not line:
            continue
        lower_line = line.lower().strip()
        if any(lower_line.startswith(prefix + ":") or lower_line == prefix for prefix in blocked_prefixes):
            continue
        line = re.sub(r"^(projects?|personal projects?|professional projects?)\s*[:\-]?\s*", "", line, flags=re.IGNORECASE).strip()
        if not line:
            continue
        if len(line.split()) < 2:
            continue
...
def extract_nationality_country(text: str) -> str:
    match = re.search(r"\bnationality\b\s*[:\-]?\s*([A-Za-z ]{3,40})", text, flags=re.IGNORECASE)
    if match:
        value = re.sub(r"\s+", " ", match.group(1)).strip(" .,:;")
        if value:
            if value.lower().startswith("indian"):
                return "India"
            return value.title()
    if re.search(r"\bindian\b|\bindia\b", text, flags=re.IGNORECASE):
        return "India"
    return ""


def extract_country_region(text: str) -> str:
    for pattern in [r"\bstate\b\s*[:\-]?\s*([A-Za-z ]{3,40})", r"\bregion\b\s*[:\-]?\s*([A-Za-z ]{3,40})"]:
...
def parse_resume_text(raw_text: str, mode: str = "balanced") -> Dict[str, object]:
    ...
    extracted_nationality = extract_nationality_country(text)
    extracted_region = extract_country_region(text)
    ...
    parsed: Dict[str, object] = {
        ...
        "religion": "",
        "marital_status": "",
        "nationality_country_name": extracted_nationality or nationality_country_name,
        "country_region": extracted_region or country_region,
        "city": city,
