...
from app.mapping_model import (
    load_mapping_model,
    save_mapping_model,
    train_mapping_model_from_dataset,
    update_mapping_model_with_rows,
)
...
AUTO_TRAIN_MODEL_ONLY = str(os.environ.get("AUTO_TRAIN_MODEL_ONLY", "false")).strip().lower() in {"1", "true", "yes", "on"}
ASYNC_DOC_LOCK = Lock()
...
def _current_dataset_entries() -> int:
    if AUTO_TRAIN_MODEL_ONLY:
        return 0
    return len(_load_json_list(DATASET_PATH))
...
def _async_inline_submit_enabled() -> bool:
    value = str(os.getenv("ASYNC_INLINE_SUBMIT_ENABLED", "false")).strip().lower()
...
async def parse_async_endpoint(
    ...
    inline_done = False
    if _async_inline_submit_enabled():
        try:
...
def retrain_mapping_endpoint(
    ...
    try:
        ...
        dataset_entries = 0
        if AUTO_TRAIN_MODEL_ONLY:
            base_model = load_mapping_model(MODEL_PATH) if payload.append_to_existing else {}
            model = update_mapping_model_with_rows(base_model, incoming)
        ...
        events["retrain_events"].append(
            {
                ...
                "total_dataset_entries": dataset_entries,
        ...
        return RetrainMappingResponse(
            dataset_path=str(DATASET_PATH) if not AUTO_TRAIN_MODEL_ONLY else "",
...
def feedback_endpoint(
    ...
    dataset = _load_json_list(DATASET_PATH) if not AUTO_TRAIN_MODEL_ONLY else []
    retrained = False
    ...
    if AUTO_TRAIN_MODEL_ONLY:
        base_model = load_mapping_model(MODEL_PATH)
        model = update_mapping_model_with_rows(base_model, train_rows) if train_rows else base_model
    else:
    ...
    events["feedback_events"].append(
        {
            ...
            "retrained": retrained,
            "total_dataset_entries": len(dataset) if not AUTO_TRAIN_MODEL_ONLY else 0,
        }
    )
    if retrained:
        events["retrain_events"].append(
...
def training_feedback_analytics(authorization: str | None = Header(default=None)) -> dict:
    ...
    return {
        ...
        "totals": {
            ...
            "current_dataset_entries": _current_dataset_entries(),
        },
...
async def auto_test_llm_colab_endpoint(
    ...
    added_entries = 0
    trained = False
    model = load_mapping_model(MODEL_PATH) if AUTO_TRAIN_MODEL_ONLY else (train_mapping_model_from_dataset(dataset) if dataset else {})
    ...
    if llm_error is None and raw_text and auto_train:
        ...
        if AUTO_TRAIN_MODEL_ONLY:
            model = update_mapping_model_with_rows(model, [train_row])
        else:
            model = train_mapping_model_from_dataset(dataset)
        save_mapping_model(model, MODEL_PATH)
        trained = True
